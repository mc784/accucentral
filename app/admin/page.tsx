...
import { adminOnboardingTour } from '@/lib/tours'
...
export default function AdminDashboard() {
  ...
  const { 
    ...
    totalSteps, 
    isActive 
  } = useTour(adminOnboardingTour);

  useEffect(() => {
    const tourHasBeenSeen = localStorage.getItem(`tour_${adminOnboardingTour.id}_completed`);
    if (!tourHasBeenSeen) {
      start();
    }
  ...
  const handleTourFinish = () => {
    localStorage.setItem(`tour_${adminOnboardingTour.id}_completed`, 'true');
    stop();
  }

  const handleRestartTour = () => {
    localStorage.removeItem(`tour_${adminOnboardingTour.id}_completed`);
    start();
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg transition underline"
                >
                  Help Tour
                </button>
        ...
        {(pendingBookings.length > 0 || pendingProviders.length > 0 || renewalAlerts.length > 0) && (
          <div id="tour-admin-alert-bar" className="bg-warm-coral text-white py-3">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {/* Revenue Card */}
            <div id="tour-admin-revenue-card" className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-sage-green">
          ...
          <div className="bg-white rounded-xl shadow-lg mb-8">
            ...
            <div className="p-6">
              {/* Overview Tab */}
              {activeTab === 'overview' && (
                <div id="tour-admin-priority-actions" className="space-y-6">
                  <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Link
              id="tour-admin-provider-approval-link"
              href="/admin/providers"
              className="block bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition border-2 border-transparent hover:border-deep-teal"
            >
              <div className="text-3xl mb-3">✅</div>
